...
export default function Exercise() {
    ...
    function getExercise() {
        ...
        axios.get('http://localhost:3001/Lang')
            .then(result => {
                setLanguageData(result.data)
            })

            if (params.id)
        axios.get(`http://localhost:3001/exercise/${params.id}`)
            .then(result => {
    ...
    function UpdateExercise(e) {
        ...
        if (params.id) {
            axios.put(`http://localhost:3001/exercise/${params.id}`, ExecriseData, { headers: { "Authorization": context.token } })
                .then((res) => { console.log(res.data) }).catch((err) => console.log(err))
        }
        else {
            axios.post(`http://localhost:3001/exercise`, ExecriseData, { headers: { "Authorization": context.token } })
                .then((res) => { console.log(res.data) }).catch((err) => console.log(err))
